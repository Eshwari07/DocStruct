...
class PdfParser(BaseParser):
    ...
    def _extract_text_for_pages(
        self, doc: "fitz.Document", page_start: int, page_end: int, *, max_chars: int = 6000
    ) -> str:
        """
    ...
    def parse(self) -> DocumentTree:
        ...
        # Pre-compute end pages: for each entry, end at the page before the next
        page_ends: List[int] = []
        for idx, start in enumerate(page_starts):
            if idx + 1 < len(page_starts):
                next_start = page_starts[idx + 1]
